defmodule LdQWeb.BCardController do
  ...
  def index(conn, _params) do
    book_cards = BCard.list_book_cards()
    render(conn, :index, book_cards: book_cards)
  end

  def new(conn, _params) do
    changeset = BCard.change_b_card(%BCard{})
    render(conn, :new, changeset: changeset)
  end

  def create(conn, %{"b_card" => b_card_params}) do
    case BCard.create_b_card(b_card_params) do
      {:ok, b_card} ->
        conn
  ...
  def show(conn, %{"id" => id}) do
    b_card = BCard.get_b_card!(id)
    render(conn, :show, b_card: b_card)
  end

  def edit(conn, %{"id" => id}) do
    b_card = BCard.get_b_card!(id)
    changeset = BCard.change_b_card(b_card)
  ...
  def update(conn, %{"id" => id, "b_card" => b_card_params}) do
    ...
    case BCard.update_b_card(b_card, b_card_params) do
      {:ok, b_card} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    b_card = BCard.get_b_card!(id)
    {:ok, _b_card} = BCard.delete_b_card(b_card)
